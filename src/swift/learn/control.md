...
但是有些时候会导致内容在非安全区中，并且因为机型不同，导致有一些内容无法看到。
...
```admonish title="碎碎念" collapsible=true
...
```admonish info title="备注" collapsible=true
备注，看不懂这玩意的源码，这东西好像直接是一个底层实现，每次点进去，感觉跟写TS一样想查看API如何实现的却发现把你引导到了一个巨长无比的API定义文档里面，还非常消耗CPU，点错了想出来还得缓几秒的那种。

盲猜用了观察者，当值发生变化的时候观察者通知UI层面开始重新渲染。~~A：这压根没必要猜啊~~
